...
def my_job():
    ...
    categories = set(
        posts.values_list("category__name_category", flat=True))  # {'Спорт', 'Игры', 'Политика', 'Технкиа'}
    subscribers = set(Category.objects.filter(
        name_category__in=categories))  # {<Category: Спорт>, <Category: Политика>, <Category: Игры>, <Category: Технкиа>}
    for s in subscribers:
        mail_users = list(Subscription.objects.filter(category=s.id).values_list('user__email', flat=True))
        ...
        msg = EmailMultiAlternatives(
            ...
            body='',
            from_email='zarbot351@yandex.ru',
            to=mail_users
        )
        msg.attach_alternative(html_content, 'text/html')
        msg.send()
        print('weekly notify send')
